...
4. CI runs three workflows, all on Groq: `tests.yml` (pytest on push/PR — full unit/integration/security suite, minus 4 **deselected pre-existing failures** that need triage; a redaction one must be fixed WITHOUT weakening redaction), `eval-on-push.yml` (6-case hospital gate), and `eval-nightly.yml` (two jobs: 35-case hospital + 28-case FHIR, the latter building its Gold from Synthea with a cache). A task is not complete if any of these would fail. When you fix a deselected test, remove its `--deselect` in `tests.yml`.
